...
class TrainingState():
    ...
    def run_epoch(self):
        ...
        for i, (mels, labels) in enumerate(self.train_dl):
            self.epoch += i/len(self.train_dl)
    ...
    def valid(self):
        ...
        if self.valid_map > self.best_valid_map:
            logger.info("Model saved in: %s", save_model(self.model))
...
def main(in_sweep=True) -> None:
    ...
    train_process = TrainingState(**training_state_args)
