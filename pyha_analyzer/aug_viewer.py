...
def get_min_max(mel_list: List[np.ndarray]) -> Tuple[float,float]:
    ...
    vmin = 0.0
    for mel in mel_list:
        vmax = max(vmax, mel.max())
        vmin = min(vmin, mel.min())
    return vmin, vmax
...
def run_test(n_clips, norms, cfg): 
    ...
    # Get audio
    audio = get_audio(train_ds, n_clips, cfg)
    # Get augs
